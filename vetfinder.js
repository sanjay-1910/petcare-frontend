...
class VetFinder {
    ...
    async fetchAutocomplete(query) {
        try {
            const response = await fetch(`${this.BASE_URL}/api/places/autocomplete?input=${encodeURIComponent(query)}`);
            const data = await response.json();
    ...
    async handleSearch() {
        ...
        try {
            ...
            const response = await fetch(`${this.BASE_URL}/api/search/veterinary`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
